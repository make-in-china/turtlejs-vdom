...
function treeEach<T, U extends keyof T>(array: T[] | IArray<T>, propertyName: U, fn: (node: T, state: ITreeEachState<T>) => (eTreeEach | void), beginIndex: number = 0): ITreeEachReturn<T> | undefined {
    ...
    while (true) {
        if (i < arr.length) {
            ...
            let childArr: T[] | IArray<T> = <any>obj2[propertyName];
            if (obj2 && childArr && childArr.length > 0 && !(ret & eTreeEach.c_noIn)) {// && propertyName) {
                stack.push(arr);
